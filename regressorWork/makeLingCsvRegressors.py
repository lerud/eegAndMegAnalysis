...
csvMainDir = "/Users/karl/Dropbox/UMD/audioMixes/mfaWork/outputs/"
...
faTargPref = "american/female/testing/testing/"
fbTargPref = "british/female/testing/testing/"
maTargPref = "american/male/testing/testing/"
mbTargPref = "british/male/testing/testing/"

faDistPref = "american/female/testing/testing/dist-"
fbDistPref = "british/female/testing/testing/dist-"
maDistPref = "american/male/testing/testing/dist-"
mbDistPref = "british/male/testing/testing/dist-"
...
predictorLabels = ["phsurp", "cohtent", "wordprob", "wordsurp"]
predictorNames = []
for i in predictorLabels:
    predictorNames.append("~" + i + ".pickle")
...
for counterbalance in range(
    ...
    for i in range(
        ...
        targetCsv = pd.read_csv(
            csvMainDir + target + str(i).zfill(2) + ".csv"
        )  # Read in the appropriate speaker's target text, which always come in order
        distractorCsv = pd.read_csv(
            csvMainDir
            + distractor
            + str(distractorOrders[counterbalance, i]).zfill(2)
            + ".csv"
        ...
        targetTimes = []
        targetValues = []
        for predictorLabel in predictorLabels:
            targetTimes.append(
                targetCsv.Begin[targetCsv[predictorLabel] != 0].to_numpy()
            )
            tempValues = targetCsv[predictorLabel][
                targetCsv[predictorLabel] != 0
            ].to_numpy()
            tempValues[np.isinf(tempValues)] = 0
            targetValues.append(tempValues)
        ...
        if not np.any(
            i == skipDist[counterbalance]
        ):  # If we're on a trial/condition with no distractor, simply don't make the predictors, and don't try to save them further down
            distractorTimes = []
            distractorValues = []
            for predictorLabel in predictorLabels:
                distractorTimes.append(
                    distractorCsv.Begin[distractorCsv[predictorLabel] != 0].to_numpy()
                )
                tempValues = distractorCsv[predictorLabel][
                    distractorCsv[predictorLabel] != 0
                ].to_numpy()
                tempValues[np.isinf(tempValues)] = 0
                distractorValues.append(tempValues)
        ...
        t = np.linspace(0, len(stimulus) / fsAudio, int(len(stimulus) * fs / fsAudio))
        ...
        targetPredictors = []
        for predictorInd in range(len(predictorLabels)):
            targetPredictor = createPredictorTimeseries(
                targetTimes[predictorInd],
                fs,
                int(len(stimulus) * fs / fsAudio),
                values=targetValues[predictorInd],
            )
            if doGaussian:
                targetPredictor = sp.ndimage.gaussian_filter1d(
                    targetPredictor, gaussianSD
                )
            targetPredictors.append(targetPredictor)
        ...
        if not np.any(
            ...
            distractorPredictors = []
            for predictorInd in range(len(predictorLabels)):

                # Usually do things to time vars first, and then values second, for consistency, but here need to use the original time var before changing it, so reverse the order
                distractorValues[predictorInd] = distractorValues[predictorInd][
                    distractorTimes[predictorInd] < stimulus.shape[0] / fsAudio
                ]
                distractorTimes[predictorInd] = distractorTimes[predictorInd][
                    distractorTimes[predictorInd] < stimulus.shape[0] / fsAudio
                ]

                distractorPredictor = createPredictorTimeseries(
                    distractorTimes[predictorInd],
                    fs,
                    int(len(stimulus) * fs / fsAudio),
                    values=distractorValues[predictorInd],
                )
                if doGaussian:
                    distractorPredictor = sp.ndimage.gaussian_filter1d(
                        distractorPredictor, gaussianSD
                    )
                distractorPredictors.append(distractorPredictor)
        ...
        for j, predictorName in enumerate(predictorNames):
            eb.save.pickle(
                targetPredictors[j],
                f"{stimMainDir}targets/predictors/{prependText[counterbalance]}_target{i+1}{predictorName}",
            )
        ...
        if not np.any(
            ...
            for j, predictorName in enumerate(predictorNames):
                eb.save.pickle(
                    distractorPredictors[j],
                    f"{stimMainDir}distractors/predictors/{prependText[counterbalance]}_distractor{i+1}{predictorName}",
                )
                eb.save.pickle(
                    targetPredictors[j] + distractorPredictors[j],
                    f"{stimMainDir}mixes/predictors/{prependText[counterbalance]}_mix{i+1}{predictorName}",
                )
        else:
            for j, predictorName in enumerate(predictorNames):
                eb.save.pickle(
